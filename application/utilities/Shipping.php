...
class Shipping
{
    ...
    /**
     * Push a paid order line to the active shipping API (e.g. Shiprocket).
     * Called automatically after payment confirmation.
     */
    public function pushOrderProductToShipping(int $opId): bool
    {
        if (1 > $opId || !Plugin::isActiveByType(Plugin::TYPE_SHIPPING_SERVICES)) {
            return false;
        }
        ...
        $opSrch->addMultipleFields([
            'op_id', 'op_order_id', 'op_invoice_number', 'op_product_type', 'op_status_id',
            'op_selprod_user_id', 'opshipping_by_seller_user_id', 'opshipping_plugin_id',
            'opshipping_service_code', 'opshipping_carrier_code', 'opshipping_fulfillment_type',
            'opship_order_number', 'plugin_code', 'order_payment_status', 'optsu_user_id'
        ]);
        $data = (array) $db->fetch($opSrch->getResultSet());
        if (empty($data)) {
            $this->error = Labels::getLabel('ERR_INVALID_ORDER', $this->langId);
            return false;
        }

        if (Product::PRODUCT_TYPE_PHYSICAL != $data['op_product_type']) {
            return false;
        }

        if (Shipping::FULFILMENT_PICKUP == ($data['opshipping_fulfillment_type'] ?? Shipping::FULFILMENT_SHIP)) {
            return false;
        }

        if (1 > FatUtility::int($data['opshipping_plugin_id']) || !empty($data['opship_order_number'])) {
            return false;
        }

        if (Orders::ORDER_PAYMENT_PAID != $data['order_payment_status']
            && 'cashondelivery' != strtolower((string) $data['plugin_code'])
        ) {
            return false;
        }

        $shippingBySeller = CommonHelper::canAvailShippingChargesBySeller(
            $data['op_selprod_user_id'],
            $data['opshipping_by_seller_user_id']
        );
        $sellerId = $shippingBySeller ? FatUtility::int($data['opshipping_by_seller_user_id']) : 0;
        $shippingApiObj = $this->getShippingApiObj($sellerId);
        if (false === $shippingApiObj) {
            SystemLog::system($this->getError(), 'Shipping-Push-' . $opId);
            return false;
        }

        if (FatUtility::int($shippingApiObj->getKey('plugin_id')) !== FatUtility::int($data['opshipping_plugin_id'])) {
            return false;
        }

        if (!method_exists($shippingApiObj, 'canGenerateLabelFromShipment')
            || false === $shippingApiObj->canGenerateLabelFromShipment()
        ) {
            return false;
        }
        ...
        if (false === $shippingApiObj->proceedToShipment($requestParam)) {
            SystemLog::plugin(json_encode($requestParam), $shippingApiObj->getError(), get_class($shippingApiObj)::KEY_NAME . '-AutoPush');
            $this->error = $shippingApiObj->getError();
            return false;
        }

        $orderInfo = $shippingApiObj->getResponse();
        $trackingNumber = $orderInfo['tracking_code'] ?? '';
        $updateData = [
            'opship_op_id' => $opId,
        ...
        if (!$db->insertFromArray(OrderProductShipment::DB_TBL, $updateData, false, [], $updateData)) {
            $this->error = $db->getError();
            SystemLog::system($this->error, 'Shipping-Push-' . $opId);
            return false;
        }
