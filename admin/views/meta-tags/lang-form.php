...
$sellerInventorySelprodId = !empty($sellerInventorySelprodId) ? FatUtility::int($sellerInventorySelprodId) : 0;
$sellerInvJsSuffix = (0 < $sellerInventorySelprodId) ? ', 0, 1' : '';

HtmlHelper::formatFormFields($langFrm);
if (0 < $sellerInventorySelprodId) {
    $langFrm->setFormTagAttribute('data-onclear', 'editSellerProductInventory(' . $sellerInventorySelprodId . ')');
} else {
...
$langFrm->setFormTagAttribute('onsubmit', 'setupLangMetaTag(document.getElementById("frmMetaTagLang"), "' . $metaType . '"); return(false);');

$langFld = $langFrm->getField('lang_id');
$langFld->setfieldTagAttribute('onChange', "editMetaTagLangForm(" . $metaId . ", this.value, '" . $metaType . "', " . $metaTagRecordId . $sellerInvJsSuffix . ");");
...
if (!empty($translatorSubscriptionKey) && $lang_id != CommonHelper::getDefaultFormLangId()) {
    ...
    $onclick = "editMetaTagLangForm(" . $metaId . ", " . $lang_id . ", '" . $metaType . "', " . $metaTagRecordId . ", 1" . (0 < $sellerInventorySelprodId ? ', 1' : '') . ")";
